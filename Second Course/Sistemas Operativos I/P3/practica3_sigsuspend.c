...
sigset_t mascara_vacia, mascara_sigusr1; //Variables globales de mascaras
...
void sigusr1(int signo){
  //Se ha omitido la variable global usada para la escucha aciva
}
...
int main(int argc, char *argv[]){
  ...
  sigemptyset(&mascara_vacia); //Mascara_vacia no contiene señales
  sigemptyset(&mascara_sigusr1);
  sigaddset(&mascara_sigusr1, SIGUSR1); //Mascara_sigusr1 solo contiene la señal SIGUSR1
  sigprocmask(SIG_SETMASK, &mascara_sigusr1, NULL); //Bloqueamos SIGUSR1
...
void consumidor(int N, opcio *opciones, reglas *reglas, int ret1, int ret2, int fd1[], int fd2[], int num_opciones, int num_reglas){
  ...
  sigprocmask(SIG_SETMASK, &mascara_vacia, NULL); //Desbloqueamos SIGUSR1
...
void productor(int N, int fd[], int num_opciones){
  ...
  for(i = 0; i < N; i++){
    sigsuspend(&mascara_vacia); //Esperamos señal
