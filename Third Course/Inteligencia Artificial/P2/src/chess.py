...
class Chess():
    ...
    def moveSim(self, start, to, verbose=True):
        ...
        if target_piece.is_valid_move(self.boardSim, start, to):
            ...
            for m in range(len(self.boardSim.currentStateW)):
                ...
                if is_end_piece and name_end_piece == self.boardSim.listNames[int(aa[2] - 1)] and end_piece_color:
                    stateEndPiece = [self.boardSim.currentStateW[m][0],self.boardSim.currentStateW[m][1],self.boardSim.currentStateW[m][2]]


            if stateEndPiece != None:
               self.boardSim.currentStateW.remove(stateEndPiece)                   
    ...
    #                   print("Next States: ",self.board.getListNextStatesW(self.board.currentStateW[m]))
                if is_end_piece and name_end_piece == self.boardSim.listNames[int(aa[2] - 1)] and not end_piece_color:
                   stateEndPiece = [self.boardSim.currentStateB[m][0],self.boardSim.currentStateB[m][1],self.boardSim.currentStateB[m][2]]

            if stateEndPiece != None:
               self.boardSim.currentStateB.remove(stateEndPiece)
